...
var starInit = function (data) {
    ...
    var labelSet = function (section) {
        switch (section) {
            ...
            case 'store-m365-uk-ua':
            case 'store-m365-uk-ww':
    ...
    setTimeout(function () {
        ...
        if (privacyStmt.length > 0) {
            ...
            switch (currentSection) {
                ...
                case 'webpurchase-sales-da-dk':
                case 'store-m365-da-dk':
                case 'office365-leadgen-da-dk':
                case 'dynamics-leadgen-da-dk':
                case 'powerplatform-presales-da-dk':
                    latestEle.innerHTML = 'Beskyttelse af personlige oplysninger og cookies';
                    break;

                case 'store-sales-fi-fi':
                case 'webpurchase-sales-fi-fi':
                case 'store-m365-fi-fi':
                case 'office365-leadgen-fi-fi':
                ...
                case 'office365-leadgen-id-id':
                    latestEle.innerHTML = 'Privasi dan Cookie';
                    break;
...
function hookAfterLinesHandler() {
    ...
    callback: function (options, successCallback, errorCallback) {
            if(lpTag.section == "store-sales-en-us-exp1" || lpTag.section == "store-m365-en-us-exp1" || lpTag.section == "store-m365-en-us" || lpTag.section == "store-sales-en-us"){
            ...
            lpTag.hooks.push({
                ...
                callback: function (options) {
                    ...
                    options.data.lines.forEach(function (line, index, array) {
                        ...
                        // or the Greeter bot only this section "store-sales-en-us-exp1"
...
lpTag.taglets.ada_enhancements_4_1 = lpTag.taglets.ada_enhancements_4_1 || (function () {
    ...
    // Begin -> Fix for accessibility issue #72813 - Keyboard focus loss is observed when we try to close the tooltip for 'Minimize the chat window', 'End the conversation' button via esc key.
        var chatWindow = document.querySelector('.lp_maximized');
        if(chatWindow){
            chatWindow.addEventListener('keydown', setInputFocus, false);
        }
    };

    var setInputFocus = function setInputFocus(e) {
        setTimeout(function(){
            if(e.keyCode == 27){
                e.preventDefault();
    ...
    // End -> Fix for accessibility issue #72813
    ...
    var actionBtnEvent = function actionBtnEvent(event) {
        var actionsBtn = document.getElementsByClassName('lp_actions_button')[0];
        var sendBtn = document.getElementsByClassName('lp_send_button')[0];
        var endConvBtn = document.querySelector('[id*="LP_EndChatAction"]').childNodes[0];
        var clearHistoryBtn = document.querySelector('[id*="LP_ForgetMeAction"]').childNodes[0];
        ...
        if (event.key == 'Tab' && !event.shiftKey || event.key == 'ArrowDown' && !event.shiftKey) {
            ...
            if (actionsBtn && actionsBtn.getAttribute('aria-expanded') == 'true') {
                ...
                if (document.activeElement == clearHistoryBtn && document.getElementById('LP_ForgetMeAction_1').nextSibling.childNodes[0].hasAttribute('disabled')){
                    event.preventDefault();
                    document.getElementsByClassName('lp_actions_button')[0].focus();
                }
            }
